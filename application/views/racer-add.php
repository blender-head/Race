...
<div id="add-wrapper">
<div id="form-racer">
    <?php echo image_asset('ngepottt.png', '', array('alt'=>'form-racer', 'width'=>801, 'height'=>311)) ?>
</div>
    	...
    	<?php echo form_open('racers/add') ?>
           ...
           <div class="form-item-add">
        		<label for="racer-class-add" id="label-racer">Additional Class : </label>
                	...
                	<?php if (sizeof($option) > 0) : ?>
                    	<?php foreach ($option as $value): ?>
                        	<p class="checkbox-class"><input name="class-additional[]" type="checkbox" id="racer-class-add" value="<?php echo $value->class_id ?>"><?php echo $value->class_name ?></p>
                        <?php endforeach ?>
                    <?php endif ?>
            </div>
